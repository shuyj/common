...
int pthread_cond_timewait_(pthread_cond_t *cond, pthread_mutex_t *mutex, int msseconds)
{
	...
	tsp.tv_nsec += (msseconds%1000) * 1000000;
	tsp.tv_sec = tsp.tv_sec + msseconds/1000 + tsp.tv_nsec / 1000000000;
	tsp.tv_nsec = tsp.tv_nsec % 1000000000;
	return pthread_cond_timedwait(cond, mutex, &tsp);
}
...
#pragma pack(pop)
int WIN_thread_sname(const char* tname)
{
	const DWORD MS_VC_EXCEPTION=0x406D1388;
	THREADNAME_INFO info;
	info.dwType = 0x1000;
	info.szName = tname;
	info.dwThreadID = ortp_thread_self();
	info.dwFlags = 0;
	__try
	{
...
int WIN_cond_timewait(ortp_cond_t * hCond, ortp_mutex_t * hMutex, int msseconds )
{
	WIN_mutex_unlock(hMutex);
	WaitForSingleObject(*hCond, msseconds);
	WIN_mutex_lock(hMutex);
	return 0;
}
...
int WIN_key_create(ortp_key_t *m, void (*cleanup)(void*))
{
	*m = TlsAlloc();
	return 0;
}
ortp_lib_t WIN_lib_open(char* lpLibFileName, int nouse )
{
