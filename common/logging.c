...
void mac_logv(const char *format, va_list args){
...
//	time_t t=0;
	...
	ortpTimeSpec ots;
//	time(&t);
	ortp_get_cur_time(&ots);
	ttm=localtime(&ots.tv_sec);

//	if (__log_file==NULL) __log_file=stderr;
	switch(lev){
		case ORTP_DEBUG:
			lname="debug";prio = ANDROID_LOG_DEBUG;
			break;
		case ORTP_MESSAGE:
			lname="message";prio = ANDROID_LOG_INFO;
			break;
		case ORTP_WARNING:
			lname="warning";prio = ANDROID_LOG_WARN;
			break;
		case ORTP_ERROR:
			lname="error";prio = ANDROID_LOG_ERROR;
			break;
		case ORTP_FATAL:
			lname="fatal";prio = ANDROID_LOG_FATAL;
			break;
		default:
			ortp_fatal("Bad level !");
	}
#ifdef ANDROID
	__android_log_vprint(prio, " 16show ", fmt, args);
	if (__log_file==NULL)
		return;
#endif
	...
	OutputDebugString("\r\n");
#endif
	fprintf(__log_file,"[%d-%d][%d:%d:%d:"FLLD"]%s-%s" ENDLINE, ttm->tm_mon+1,ttm->tm_mday, ttm->tm_hour,
		ttm->tm_min,ttm->tm_sec,ots.tv_nsec/1000000,lname,msg);
	fflush(__log_file);
	ortp_free(msg);
}
